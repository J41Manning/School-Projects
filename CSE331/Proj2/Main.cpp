...
int main(int argc, char *argv[])
{
  ...
  CQuickSort<CMoviePointer> isort;
  CVerifySort<CMoviePointer> vsort;
  CDuplicatesRemove<CMoviePointer> dr;
  ...
  while(ti < 600){
    ...
    std::cout << "Verified    : " << boolalpha <<vsort.VerifySort(movies) << std::endl;
    dr.DuplicatesRemove(movies);
    std::cout << "Unque movies: " << movies->size() << std::endl;
