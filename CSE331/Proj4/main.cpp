...
#include "CTAGradeHook.h" //a hook for the TA to add testing code.  DO NOT REMOVE!
...
bool CActors::TreeLoad(const char *p_filename, int p_limit)
{
    ...
    cout << "All actors loaded" << endl;
    cout << "There are " << (int)m_tempActors.size() << " actors" << endl;

    //
    // Insert actors into the tree
    // Sort them first...
    //

    std::sort(m_tempActors.begin(), m_tempActors.end());

    InsertActorsTree(0, (int)m_tempActors.size()-1);
    m_tempActors.clear();

    cout << "Actor Tree loaded" << endl;
    cout << "Actor Tree depth = " << m_actors.Depth() << endl;

    //
    // Pass 2:  Load the movies and create all of the cross-references....
    //
...
//
// Name :         CActors::InsertActorsTree()
// Description :  Recursive function to load the actors into the tree in a balanced fashion.
//

void CActors::InsertActorsTree(int p_first, int p_last)
{
    if(p_first > p_last)
        return;

    int med = (p_last + p_first) / 2;

    m_actors.Insert(m_tempActors[med]);
    InsertActorsTree(p_first, med-1);
    InsertActorsTree(med+1, p_last);
}
...
void CActors::ActorMovie(std::string &p_actor, std::string &p_movie)
{
    ...
    {
        //
        // Pass 1:  Loading the actors
        //

        // Simple test for the duplicate actors called here...
        if(p_actor != m_lastActor)
        {
            m_tempActors.push_back(p_actor);
            m_lastActor = p_actor;
        }
